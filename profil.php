...
if (!isset($_SESSION['loggedin']) || $_SESSION['loggedin'] !== true) {
    $_SESSION['error_messages'][] = "Kamu harus log-in terdahulu";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['profile_photo'])) {
    ...
    $placeholderImage = "./image/photo-placeholder.png"; // Path to the placeholder image

    try {
        // Check if the file is an actual image and the path is not empty
        if (!empty($_FILES["profile_photo"]["tmp_name"])) {
            $check = getimagesize($_FILES["profile_photo"]["tmp_name"]);
            if ($check === false) {
                throw new Exception("File tersebut bukan gambar");
            }
        } else {
            // If the file path is empty, set the target file to the placeholder image
            $targetFile = $placeholderImage;
        }

        // Allow only certain file formats
        if ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $targetFile !== $placeholderImage) {
            throw new Exception("Hanya boleh file JPEG, JPG, dan PNG untuk gambar");
        }

        // Check if everything is ok before uploading
        if ($uploadOk == 1 && $targetFile !== $placeholderImage) {
            if (move_uploaded_file($_FILES["profile_photo"]["tmp_name"], $targetFile)) {
                ...
                if ($stmt->execute()) {
                    $_SESSION['error_messages'][] = "Foto berhasil di perbarui.";
                } else {
                    throw new Exception("Ada kesalahan memperbarui foto: " . $stmt->error);
                }
                $stmt->close();
            } else {
                throw new Exception("Ada kesalahan mengupload file.");
            }
        } else {
            // If no file is uploaded, update the database with the placeholder image
            $student_id = $_SESSION['student_id'];
            $sql = "UPDATE users SET photo_path = ? WHERE student_id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("si", $targetFile, $student_id);
            if ($stmt->execute()) {
                $_SESSION['error_messages'][] = "Tidak ada gambar yang di upload. Gambar sementara di pasang.";
            } else {
                throw new Exception("Ada kesalahan memperbarui foto: " . $stmt->error);
            }
            $stmt->close();
        ...
        $_SESSION['error_messages'][] = "Ada Kesalahan: " . $e->getMessage();
    }
}
...
if ($stmt = $conn->prepare($sql)) {
    ...
    $_SESSION['error_messages'][] = "Ada kesalahan mengambil data user.";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['sticker'])) {
    ...
    if (empty($_FILES["sticker"]["tmp_name"])) {
        $_SESSION['error_messages'][] = "Tidak ada file yang di upload.";
        $uploadOk = 0;
    } else {
        $check = getimagesize($_FILES["sticker"]["tmp_name"]);
        if ($check === false) {
            $_SESSION['error_messages'][] = "File bukan gambar.";
            $uploadOk = 0;
        }
    }

    if ($imageFileType != "png") {
        $_SESSION['error_messages'][] = "Hanya boleh file PNG untuk sticker.";
        $uploadOk = 0;
    }

    if ($uploadOk == 0) {
        $_SESSION['error_messages'][] = "Filemu tidak terupload";
    } else {
        if (move_uploaded_file($_FILES["sticker"]["tmp_name"], $targetFile)) {
            ...
            $_SESSION['error_messages'][] = "Ada kesalahan mengupload file.";
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['sticker_id']) && isset($_POST['sticker_width']) && isset($_POST['sticker_height']) && isset($_POST['sticker_z_index'])) {
        ...
        if ($stmt = $conn->prepare($query)) {
            ...
            if ($stmt->execute()) {
                $_SESSION['error_messages'][] = "Sticker berhasil di perbarui, check di kiri atas jika tidak ketemu";
            } else {
                $_SESSION['error_messages'][] = "Ada kesalahan memperbarui Sticker: " . $stmt->error;
            }
            $stmt->close();
        } else {
            $_SESSION['error_messages'][] = "Ada kesalahan: " . $conn->error;
        }
    }
}
...
<head>
    ...
    <style>
        ...
        .profile-container {
            ...
            min-height: 100vh;
            gap: 30px;
            background-color: rgba(245, 242, 251, 0.36);
            padding: 20px;
            margin: 20px;
            border-radius: 8px;
            box-shadow: 0 0 10px rgba(17, 14, 22, 0.1);
        }
        ...
        .profile-info table td {
            text-align: left;
            padding: 8px;
            border-bottom: 2px solid rgba(36, 6, 84, 0.226);
        }
        ...
        .profile-background-image {
            ...
            top:960px;
            bottom:50px;
            left: 600px; /* Align with the left side */
...
<body>
                ...
                <form method="POST" enctype="multipart/form-data" action="profil.php">
                    <div class="profile-photo" style="text-align: center;">
                        ...
                        <img id="profile-photo" src="<?php echo htmlspecialchars($photo_path); ?>" alt="Profil Foto" 
                            style="max-width: 100%; height: auto; border-radius: 15px; border: 2px solid #ccc; display: inline-block; margin-bottom: 30px; z-index:1;position:relative;">
                        <br>
    ...
    <div id="error-modal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.7); color: black; padding: 20px; z-index: 50;">
        <div style="background: rgba(255, 255, 255, 0.9) url('image/error-background.png') center center / cover no-repeat; padding: 20px; border-radius: 8px; min-width: 500px; min-height: 300px; margin: 100px auto; position: relative;">
            <button id="close-modal" style="position: absolute; top: 10px; right: 10px; background: red; border: none; color: white; padding: 5px 10px; cursor: pointer;">Tutup</button>
            <h2>Sepertinya..</h2>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        window.addEventListener('load', function() {
            ...
            profilePhoto.style.height = '400px';
            profilePhoto.style.objectFit = 'cover'; // Ensures the image covers the area without distorting
        });
